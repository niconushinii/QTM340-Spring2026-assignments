...
```{R}
# 1. Filter for the Chancellors (more robust than exact string matching)
chancellors <- hansard_1830 %>%
  filter(!is.na(text)) %>%
  mutate(speaker = stringr::str_squish(speaker)) %>%
  filter(str_detect(
    speaker,
    regex("\\bViscount\\s+Althorp\\b|\\bMr\\s+Spring\\s+Rice\\b|\\bMr\\s+Baring\\b", ignore_case = TRUE)
  ))

# 2. Tokenize, count, and calculate TF-IDF
chancellors_tfidf <- chancellors %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "^[a-z]+$")) %>%                 # keeps only alphabetic tokens
  anti_join(stop_words, by = "word") %>%                   # explicit join key
  count(speaker, word, sort = TRUE) %>%
  bind_tf_idf(term = word, document = speaker, n = n)

# 3. Extract the top 10 most distinctive words per speaker
...
# 4. Create the facet-wrapped bar chart
ggplot(top_chancellor_words, aes(x = tf_idf, y = word)) +
...
# This plot answers: which terms most distinguish each Chancellor’s language from the other Chancellors’ speech in the 1830s?
# 3. 
```
