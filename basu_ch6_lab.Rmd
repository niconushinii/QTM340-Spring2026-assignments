...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, cache.lazy = FALSE)
```
...
annotated_1820 <- hansard_1820 %>%
  ...
  left_join(speaker_metadata_1820, by = "sentence_id") %>%
  select(text, year, speechdate, speaker, suggested_speaker)
...
annotated_1840 <- hansard_1840 %>%
  left_join(debate_metadata_1840, by = "sentence_id") %>%
  mutate(year = year(speechdate)) %>%
  left_join(speaker_metadata_1840, by = "sentence_id") %>%
  select(text, year, speechdate, speaker, suggested_speaker)
...
both_periods_word_count_totals <- both_periods_word_count %>%
  group_by(period) %>%
  summarize(total = sum(as.double(wordcount), na.rm = TRUE))
```
...
prereform_only_speakers <- prereform_speakers_wordcount %>%
  ...
  anti_join(postreform_speakers_wordcount %>% distinct(suggested_speaker),
            by = "suggested_speaker") %>%
  mutate(class = "prereform only")

# inspect the data
head(prereform_only_speakers, 10)
```
...
postreform_only_speakers <- postreform_speakers_wordcount %>% 
  distinct(suggested_speaker) %>%
  anti_join(prereform_speakers_wordcount %>% distinct(suggested_speaker),
            by = "suggested_speaker") %>%
...
continuous_speakers <- prereform_speakers_wordcount %>%
  distinct(suggested_speaker) %>%
  inner_join(postreform_speakers_wordcount %>% distinct(suggested_speaker),
             by = "suggested_speaker") %>%
...
all_classes <- bind_rows(
  prereform_only_speakers,
  postreform_only_speakers,
  continuous_speakers
) %>%
...
annotated_1820 <- hansard_1820 %>%
  ...
  mutate(
    speechdate = as.Date(as.numeric(speechdate), origin = "1970-01-01"),
...
speaker_key <- bind_rows(prereform, postreform) %>%
  select(speaker, suggested_speaker) %>%
  filter(!is.na(speaker), speaker != "",
         !is.na(suggested_speaker), suggested_speaker != "") %>%
  group_by(suggested_speaker) %>%
  sample_n(1) %>%
  ungroup()
...
```{R, cache=TRUE}
# 1. Create the classification counts for the prereform period
prereform_words_w_speaker_classifications_counted <- prereform_words %>%
  filter(!str_detect(word, "^[0-9]+$")) %>% 
  anti_join(stop_words, by = "word") %>%
  mutate(word = lemmatize_words(word)) %>%
  left_join(
    all_classes %>% distinct(suggested_speaker, .keep_all = TRUE),
    by = "suggested_speaker"
  ) %>%
  filter(class %in% c("prereform only", "both periods")) %>%
  group_by(class, word) %>%
  summarize(wordsperperiod = n(), .groups = "drop")

# 2. Calculate the actual TF-IDF
...
all_words_cleaned <- bind_rows(prereform_words, postreform_words) %>%
  filter(!str_detect(word, "^\\s*[0-9]*\\s*$")) %>%
  anti_join(stop_words, by = "word")

word_lemmas <- all_words_cleaned %>%
  distinct(word) %>%
  mutate(word_lemma = lemmatize_words(word))

all_words_cleaned <- all_words_cleaned %>%
  left_join(word_lemmas, by = "word") %>%
  mutate(word = word_lemma) %>%
  select(-word_lemma)
...
# This combines prereform and postreform words so we can compare prereform-only and postreform-only speakers across the full 1832 analysis.
...
# FIX ONLY THE METADATA DATES
debate_metadata_1850$speechdate <- as.Date(as.character(debate_metadata_1850$speechdate))
debate_metadata_1860$speechdate <- as.Date(as.character(debate_metadata_1860$speechdate))
debate_metadata_1870$speechdate <- as.Date(as.character(debate_metadata_1870$speechdate))
...
hansard <- bind_rows(annotated_1850, annotated_1860, annotated_1870)
```

```{R}
# make two categories, pre- and post-reform
prereform <- hansard %>%
  filter(speechdate >= as.Date("1859-01-01"),
         speechdate <  as.Date("1869-01-01")) %>%
  mutate(year = year(speechdate)) %>%
  mutate(suggested_speaker = coalesce(suggested_speaker, speaker)) %>%
  mutate(period = "prereform") 

postreform <- hansard %>%
  filter(speechdate >= as.Date("1869-01-01"),
         speechdate <= as.Date("1879-01-01")) %>%
  mutate(year = year(speechdate)) %>%
  mutate(suggested_speaker = coalesce(suggested_speaker, speaker)) %>%
  mutate(period = "postreform")# the "coalesce" function replaces a NA in the suggested_speaker column with whatever is in the speaker column
  #filter(!is.na(speaker)) %>% ## a different count will result if we use this line as an alternative to the above
...
all_per_speaker_wordcounts <- bind_rows(
...
top_speakers <- all_speakers_annotated %>%
  group_by(class, suggested_speaker) %>%
  summarize(wordcount_per_speaker = sum(wordcount), .groups = "drop") %>%
  group_by(class) %>%
  arrange(desc(wordcount_per_speaker), .by_group = TRUE) %>%
  slice_head(n = 10) %>%
  mutate(top = TRUE) %>%
  select(suggested_speaker, class, top)
...
ggplot(graph_ready, aes(x = year, y = wordcount, color = class, shape = class)) +
  ...
  theme(legend.position = "bottom") +
  labs(
    title = "How Much Did Speakers Speak",
...
all_words_cleaned <- bind_rows(
  prereform_words %>% anti_join(stop_words, by = "word"),
  postreform_words %>% anti_join(stop_words, by = "word")
) %>%
...
all_words_cleaned <- all_words_cleaned %>%
  left_join(word_lemmas, by = "word") %>%
  mutate(word = word_lemma) %>%
  select(-word_lemma)# lemmatize the word, reducing each word to its word stem for counting 
...
prereform_words_cleaned <- prereform_words %>%
  filter(!grepl("^[0-9]+$", word)) %>% # remove all number-only tokens
  anti_join(stop_words, by = "word") %>% # remove stopwords 
...
postreform_words_cleaned <- postreform_words %>%
  filter(!grepl(word,"^\\s*[0-9]*\\s*$")) %>% # remove all numbers 
  anti_join(stop_words, by = "word") %>% # remove stopwords 
#  anti_join(custom_stop_words, by = "word")  %>% # remove any words in the custom_stop_words list
  mutate(word = lemmatize_words(word))  # lemmatize the word, reducing each word to its word stem for counting 
...
Keeping the original names helps maintain historical accuracy and shows how speakers were listed in *Hansard* at the time. The original speaker field keeps nineteenth-century habits like titles, abbreviations, and inconsistencies, which are part of the record. But for analysis, `suggested_speaker` metadata is needed because the same person might appear under different titles, spellings, or name versions. Using only raw names could mean counting one person more than once. Metadata also stops different people from being grouped under a single office title. This way, each person is counted only once, making the analysis more accurate, while the visualization still displays the original text.
...
For the tf-idf by time period exercise, I split the data into pre-reform and post-reform groups, tokenized the text, and counted how often each word appeared in each period. Then I used tf-idf to find words that stood out in each group. Because tf-idf lowers the importance of words common to both groups, it highlights words mostly used in just one period. This method shows which words are especially distinctive to pre-reform or post-reform parliamentary speech.

### b) log likelihood by time period

For the log-likelihood by time period exercise, I also split the data into pre-reform and post-reform groups and counted word frequencies for each group. Unlike tf-idf, which focuses on words that are almost exclusive to one group, log-likelihood uses a broader definition of distinctiveness. It finds words used more often in one period than the other, even if they appear in both. This method usually gives a wider and often more historically interpretable view of language change, though it can also highlight very common words.

### c) comparing speakers from one period only against those continuously elected

For the exercise comparing speakers from only one period with those who spoke in both, I grouped the data by speaker type instead of by time. I created three groups: speakers who spoke only before the reform, only after, and those who spoke in both periods. Then I compared the words used by speakers from just one period to those who appeared in both. This helps show whether language changes happened because of the passage of time or because new speakers joined the parliamentary record. Appearing in both periods serves as a proxy for continuity, since the record shows who spoke, not who was continuously elected.
...
In the exercise comparing the most prolific speakers before and after the reform, I focused only on the most active speakers in each group. This reduces noise from less active speakers and makes it clearer whether the dominant voices before and after the reform used different language. It also helps test whether the broader patterns in the chapter were mostly being shaped by the most prominent speakers.
...
prereform_words_w_speaker_classifications_ll <- log_likelihood(
  prereform_words_w_speaker_classifications_counted %>%
    filter(!is.na(class)),
  group = "class",
  word = "word",
  n = "wordsperperiod"
...
prereform_top_words_w_speaker_classifications_ll <- prereform_words_w_speaker_classifications_ll %>%
  group_by(class) %>%
  arrange(desc(ll), .by_group = TRUE) %>%
  slice_head(n = 15) %>%
  mutate(word = reorder_within(word, ll, class))

prereform_top_words_w_speaker_classifications_ll$class <- factor(
  prereform_top_words_w_speaker_classifications_ll$class,
  levels = c("prereform only", "postreform only", "both periods")
)

ggplot(prereform_top_words_w_speaker_classifications_ll, aes(
  x = word,
  y = ll,
  fill = factor(ll)
)) +
  ...
  guides(fill = "none") +
  labs(
    title = "Which Words Are Most Distinctive (Log Likelihood)",
    subtitle = "of the prereform only speakers vs those who also spoke after the Reform Act?",
    y = "log likelihood"
  )

postreform_words_w_speaker_classifications_ll <- log_likelihood(
  postreform_words_w_speaker_classifications_counted %>%
    filter(!is.na(class)),
  group = "class",
  word = "word",
  n = "wordsperperiod"
...
postreform_top_words_w_speaker_classifications_ll <- postreform_words_w_speaker_classifications_ll %>%
  group_by(class) %>%
  arrange(desc(ll), .by_group = TRUE) %>%
  slice_head(n = 15) %>%
  mutate(word = reorder_within(word, ll, class))

postreform_top_words_w_speaker_classifications_ll$class <- factor(
  postreform_top_words_w_speaker_classifications_ll$class,
  levels = c("prereform only", "postreform only", "both periods")
)

ggplot(postreform_top_words_w_speaker_classifications_ll, aes(
  x = word,
  y = ll,
  fill = factor(ll)
)) +
  ...
  labs(
    title = "Which Words Are Most Distinctive (Log Likelihood)",
    subtitle = "of the postreform only speakers vs those who also spoke before the Reform Act?",
    y = "log likelihood"
  )
```

Log-likelihood results are usually a bit easier to interpret than tf-idf results, but not always. Tf-idf uses stricter rules and often highlights rare names, places, and very distinctive terms. Log likelihood is more flexible and can detect words used at different rates in each group, even if they appear in both groups. In my analysis, the most distinctive words were not broad political topics but aristocratic titles and formal parliamentary language, like “noble,” “lordship,” “earl,” “friend,” and “duke.” This makes the speakers’ social register more visible, but it also shows that log likelihood can still produce repetitive or hard-to-interpret results. Overall, it was a bit easier to see broader patterns with log likelihood than with tf-idf, but the results were still not completely clear.
...
My favorite visualization was the chart comparing the most distinctive words used by prereform-only speakers and those who spoke in both periods. I am a bit biased, since seeing terms like “quicksilver,” “stork,” and “Walsall” stand out made me more interested in the debates. In both the 1832 and 1867 analyses, the same pattern appears: tf-idf tends to highlight very specific and sometimes obscure words instead of broad political themes. Many of these are proper nouns, place names, or local references, showing that these groups often seem distinctive because of their links to certain places, people, and issues.

One major difference between the two events is in the words that stand out and the political contexts they reflect. In the 1832 chart, words like “quicksilver,” “stork,” and “Walsall” make the results especially unusual and specific, while the 1867 chart highlights a different political and geographic scene. This matters because it shows that even with the same method, each reform event produces its own distinctive vocabulary and concerns. The two visualizations may look similar, but the historical worlds they reveal are quite different.
...
search_1832 <- bind_rows(hansard_1820, hansard_1830) %>%
  left_join(bind_rows(debate_metadata_1820, debate_metadata_1830), by = "sentence_id") %>%
  left_join(bind_rows(speaker_metadata_1820, speaker_metadata_1830), by = "sentence_id") %>%
  mutate(speechdate = as.Date(as.character(speechdate))) %>%
  filter(speechdate >= as.Date("1822-06-04"), speechdate <= as.Date("1832-06-04")) %>%
  mutate(doc_id = row_number())

texts <- search_1832$text
names(texts) <- as.character(search_1832$doc_id)

wolverhampton_kwic <- kwic(
  tokens(texts, remove_punct = TRUE),
  pattern = "wolverhampton",
  window = 8
) %>%
  as.data.frame() %>%
  mutate(doc_id = as.integer(docname)) %>%
  left_join(
    search_1832 %>% select(doc_id, suggested_speaker, speechdate),
    by = "doc_id"
  ) %>%
  select(suggested_speaker, speechdate, pre, keyword, post)

wolverhampton_kwic %>%
  head(5)
```

“Wolverhampton” seems like a fairly obscure word at first, but KWIC shows that it appears in debates about representation and the elective franchise, alongside places like Birmingham, Manchester, Sheffield, Glasgow, and Staffordshire. This makes the place-name historically meaningful rather than random. The context shows that these debates concerned which towns and groups were considered politically represented, including industrial areas, petitioning groups, and commercial communities. Distinctiveness measures help reveal the local geography of reform by highlighting the specific places contemporaries mentioned when arguing about representation and exclusion. In short, the metric highlights the constituencies and regional interests at the heart of the reform debates, showing that the Reform Act was also about which places and populations were becoming politically visible.
...
Relying on just one computational method in historical analysis can be risky because each algorithm brings certain aspects of historical change into focus while pushing others into the background. As this chapter explains, tf-idf tends to highlight very distinctive words, while log-likelihood captures broader differences in word usage. If historians use only one method and treat its results as definitive, they might overlook important patterns or confuse mathematical oddities with real historical trends. Dividing the data by speaker type, such as those who served only before reform and those who appeared in both periods, offers another way to interpret the findings. This helps show whether changes in language came from established politicians shifting their rhetoric or from new voices entering Parliament.

Using more than one method reflects the critical approach to algorithms described in The Dangerous Art of Text Mining. Algorithms do not simply provide objective historical facts; they are interpretive tools that highlight certain things according to their own rules. By comparing results from different methods, historians can see whether a pattern holds across several measures or whether it only appears with one formula. This leads to more careful, more transparent analysis and helps prevent overconfident claims.

We often get the best insights by looking at events from different angles. Studying the 1832 Reform Act with different methods and speaker groups helps show who was involved, how language shifted, and whether those changes came from new members or from returning members changing their rhetoric. With that fuller understanding, comparing 1832 to 1867 becomes even more meaningful. This broader view helps show whether the patterns were unique to one Parliament or part of a larger trend in democratic change. Because every step in computational analysis involves choices and interpretation, using several methods and looking across different events helps prevent oversimplification. This turns distant reading into a process of testing, refinement, and careful historical judgment rather than a single automated answer.

## AI Use statement

**Tool(s) used:** ChatGPT

**Purpose(s):** To fix version-control errors in the chapter so the code would run, to correct date issues, to clarify the difference between tf-idf and log likelihood, and to help identify places where I needed to verify claims against the chapter and my own charts.

**Example prompts**: “What is causing this vec error?”
“Why can’t I install.packages("dhmeasures") directly?”
“Why won’t this lemmatize?”

**Tasks performed independently by me**: I ran the code, reviewed the charts, checked the relevant passages from the course materials, decided which interpretations fit my actual outputs, and wrote and revised the final answers in my own words.
