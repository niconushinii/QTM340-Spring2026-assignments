...
# FIX UPSTREAM: Force the corrupt IDate columns into standard Dates
debate_metadata_1830$speechdate <- as.Date(debate_metadata_1830$speechdate)
debate_metadata_1860$speechdate <- as.Date(debate_metadata_1860$speechdate)
...
From our analysis, we can start to interpret what was trending and what was going away from the vantage point of an objective observer who had lived through the years 1840 to 1859.  On the up-and-up were Ireland, corn, trade, and duties, and discussions of baronets and the poor. 
...
The causes and implications of language changes are interesting to contemplate, and, as with much work in text mining, they require deeper reading of context in order to dive deeper.  The point is that distance measurement lends us a metric to make concete phenomena that are otherwise invisible -- the pulse of rising and falling trends over time.  
...
chancellors <- hansard_1830 %>%
  left_join(speaker_metadata_1830, by = "sentence_id") %>%
  filter(!is.na(text), suggested_speaker != "") %>%
  filter(str_detect(speaker, regex("\\bchancellor\\b.*\\bexchequer\\b", ignore_case = TRUE))) %>%
  filter(str_detect(suggested_speaker, "_\\d+$")) %>%
  filter(!str_detect(speaker, regex("\\band\\b", ignore_case = TRUE)))

chancellors_tfidf <- chancellors %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "^[a-z]+$")) %>%    
  anti_join(stop_words, by = "word") %>%
  count(suggested_speaker, word, sort = TRUE) %>%
  bind_tf_idf(term = word, document = suggested_speaker, n = n)

top_chancellor_words <- chancellors_tfidf %>%
  group_by(suggested_speaker) %>%
  slice_max(tf_idf, n = 10, with_ties = FALSE) %>%
  ungroup() %>%
  mutate(word = reorder_within(word, tf_idf, suggested_speaker))

ggplot(top_chancellor_words, aes(x = word, y = tf_idf, fill = suggested_speaker)) +
  geom_col(show.legend = FALSE) +
  scale_x_reordered() +
  coord_flip() +
  facet_wrap(~ suggested_speaker, scales = "free") +
  labs(
    title = "Distinctive Words of 1830s Chancellors (TF-IDF)",
    x = "word",
    y = "TF-IDF"
  ) +
  theme_minimal()
...
This tf-idf plot highlights which words are the most distinctive in each Chancellor’s speeches compared to the other Chancellors in the 1830s subset. For example, Rice’s top words include administrative and financial terms like “banks,” “loan,” and “postage.” Goulburn’s most distinctive words are related to places and diplomacy, such as “consular” and “dover.” These results suggest different policy emphases, but a closer reading of these specific terms in context would be required to confirm any specific trade or diplomatic strategies.
...
slavery_candidates <- hansard_1830 %>%
  left_join(debate_metadata_1830, by = "sentence_id") %>%
  mutate(year = year(speechdate)) %>%
  filter(year == 1833) %>%
  filter(str_detect(debate, regex("abolition.*slav|slav.*abolition", ignore_case = TRUE))) %>%
  count(debate, sort = TRUE)

debate_title <- slavery_candidates %>% slice(1) %>% pull(debate)
debate_title

slavery_debate <- hansard_1830 %>%
  left_join(speaker_metadata_1830, by = "sentence_id") %>%
  left_join(debate_metadata_1830,  by = "sentence_id") %>%
  mutate(year = year(speechdate)) %>%
  filter(year == 1833, debate == debate_title) %>%
  filter(!is.na(speaker), suggested_speaker != "", !is.na(text))

speaker_freqs <- slavery_debate %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "^[a-z]+$")) %>%              
  anti_join(stop_words, by = "word") %>%               
  count(suggested_speaker, word) %>%
  group_by(suggested_speaker) %>%
  mutate(total_words = sum(n)) %>%
  filter(total_words >= 150) %>%   
  group_by(word) %>%
...
freq_matrix <- speaker_freqs %>%
  select(suggested_speaker, word, freq) %>% 
  pivot_wider(names_from = word, values_from = freq, values_fill = 0) %>%
  column_to_rownames("suggested_speaker") %>%
  as.matrix()
...
jsd <- function(p, q) {
  ...
  kl_div <- function(a, b) {
    ...
    b <- ifelse(b == 0, 1e-10, b)
    sum(a * log2(a / b)) }
  
  0.5 * kl_div(p, m) + 0.5 * kl_div(q, m) }

speaker_names <- rownames(freq_matrix)
n_speakers <- nrow(freq_matrix)

jsd_matrix <- matrix(0, n_speakers, n_speakers)
rownames(jsd_matrix) <- speaker_names
colnames(jsd_matrix) <- speaker_names
...
jsd_pairs <- crossing(i = 1:n_speakers, j = 1:n_speakers) %>%
  filter(i < j) %>%
  mutate(
    speaker_a = speaker_names[i],
    speaker_b = speaker_names[j],
    jsd = jsd_matrix[cbind(i, j)]
  ) %>%
  arrange(jsd)

kable(head(jsd_pairs, 5), digits = 3,
      caption = "Most Similar Speakers (Lowest JSD)") %>%
  kable_styling(latex_options = "HOLD_position")

kable(tail(jsd_pairs, 5), digits = 3,
      caption = "Most Different Speakers (Highest JSD)") %>%
  kable_styling(latex_options = "HOLD_position")
```

```{r closeread}
pull_samples <- function(df, suggested_speaker_string, n = 5) {
  df %>%
    filter(suggested_speaker == suggested_speaker_string) %>%
    select(speaker, suggested_speaker, text) %>%
    slice_sample(n = n) %>%
    mutate(text = str_trunc(text, 240))
}

slavery_debate_ids <- slavery_debate

# MOST SIMILAR pair 
similar_a <- jsd_pairs %>% slice(1) %>% pull(speaker_a)
similar_b <- jsd_pairs %>% slice(1) %>% pull(speaker_b)

bind_rows(
  pull_samples(slavery_debate_ids, similar_a, n = 5),
  pull_samples(slavery_debate_ids, similar_b, n = 5)
) %>%
  arrange(speaker) %>%
  kable(caption = "Close reading samples - most similar JSD pair", booktabs = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")

# MOST DIFFERENT pair 
different_a <- jsd_pairs %>% slice(n()) %>% pull(speaker_a)
different_b <- jsd_pairs %>% slice(n()) %>% pull(speaker_b)

bind_rows(
  pull_samples(slavery_debate_ids, different_a, n = 5),
  pull_samples(slavery_debate_ids, different_b, n = 5)
) %>%
  arrange(speaker) %>%
  kable(caption = "Close reading samples - most different JSD pair", booktabs = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
```
JSD is a helpful tool for data mining and a way to measure how similar or different each speaker’s vocabulary is in the 1833 abolition debate. A low JSD shows that two speakers use more similar words, while a high JSD means their vocabularies are less alike.  

In my results, we see that Mr. Secretary Stanley (edward_stanley_2326) and Sir Robert Peel (robert_peel_1664) are the most similar speakers, with a low JSD of about 0.249. This means that their vocabulary profiles are quite similar in this debate. On the other hand, Mr. Herries (john_herries_3008) and Colonel Davies (thomas_davies_3246) have the greatest distance, with a high JSD of 0.802, which tells us Davies’s vocabulary is much less like Herries’s in this debate. However, JSD values alone do not explain why the language is different, so I used the closest and farthest pairs to guide my close reading of the debate.

A close reading of their sentences shows that this similarity likely stems from a shared use of highly formal, procedural parliamentary language. Both speakers refer to larger groups and institutions, such as the House, petitioners, resolutions, and committee procedure, rather than focusing on specific policy details. Stanley often discusses the House’s actions (“the unanimous feeling of the House”), while Peel references how Parliament and petitioners are positioned (“the majority of the petitioners… or the majority of the House itself”) and speaks about the register of resolutions and debate management. 

Meanwhile, the greater distance between Colonel Davies and Mr. Herries shows that they focus on different topics and use different reasoning. Herries’s language is more about fiscal and constitutional issues, focusing on the wording of clauses (“he called upon him to give a more minute explanation of the grounds on which he had framed that clause in the way in which it now stood.”) and the proper mechanism for raising funds (“to come down to Parliament and demand whatever sums they might require, to meet it”).  Davies, in comparison, talks more about the economic consequences for planters (“to increase that duty would inevitably be, to increase the difficulties of the planters”) and takes a more moral stance on slavery (“he begged to repeat that there was no warmer friend than himself to the immediate abolition of slavery, as far as that was consistent with real justice and humanity”).

Overall, these patterns show that JSD distances highlight real differences in what speakers focus on, but they are just a starting point. Short excerpts can be misleading, so it is important to read more of each speaker’s words before making strong historical claims.
...
cotton_pattern <- regex("\\bcotton\\b", ignore_case = TRUE)

h1830 <- hansard_1830 %>%
  left_join(debate_metadata_1830, by = "sentence_id") %>%
  mutate(speechdate = as.Date(speechdate), year = year(speechdate)) %>%
  filter(year %in% 1830:1839, !is.na(text), !is.na(debate)) %>%
  select(debate, text) %>%
  mutate(decade = "1830s")
...
h1860 <- hansard_1860 %>%
  ...
  mutate(speechdate = as.Date(speechdate), year = year(speechdate)) %>%
  filter(year %in% 1860:1869, !is.na(text), !is.na(debate)) %>%
  select(debate, text) %>%
  mutate(decade = "1860s")

cotton_debates_1830 <- h1830 %>%
  group_by(debate) %>%
  summarize(has_cotton = any(str_detect(text, cotton_pattern)), .groups = "drop") %>%
  filter(has_cotton) %>%
  select(debate)

cotton_debates_1860 <- h1860 %>%
  group_by(debate) %>%
  summarize(has_cotton = any(str_detect(text, cotton_pattern)), .groups = "drop") %>%
  filter(has_cotton) %>%
  select(debate)

cotton_corpus <- bind_rows(
  h1830 %>% semi_join(cotton_debates_1830, by = "debate"),
  h1860 %>% semi_join(cotton_debates_1860, by = "debate")
)

cotton_tfidf <- cotton_corpus %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "^[a-z]+$")) %>%
...
cotton_tfidf %>%
  inner_join(stop_words %>% distinct(word), by = "word") %>%
  arrange(desc(tf_idf)) %>%
  slice_head(n = 10) %>%
  select(word, decade, n, tf_idf) %>%
  kable(format = "latex", booktabs = TRUE,
        caption = "Stopwords with the highest TF-IDF in cotton-related debates")

top_cotton_tfidf <- cotton_tfidf %>%
  group_by(decade) %>%
  slice_max(tf_idf, n = 20, with_ties = FALSE) %>%
  ungroup() %>%
  mutate(word = reorder_within(word, tf_idf, decade))

ggplot(top_cotton_tfidf, aes(x = word, y = tf_idf, fill = word)) +
  geom_col(show.legend = FALSE) +
  scale_x_reordered() +
  coord_flip() +
  facet_wrap(~ decade, scales = "free_y") +
  labs(
    title = "Most Distinctive Words in Cotton-Related Debates: 1830s vs 1860s",
    x = NULL,
    y = "TF-IDF"
  )

cotton_stop_counts <- cotton_corpus %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "^[a-z]+$")) %>%
  anti_join(stop_words, by = "word") %>%
  count(decade, word, sort = TRUE)

top_cotton_stop_counts <- cotton_stop_counts %>%
  group_by(decade) %>%
  slice_max(n, n = 10, with_ties = FALSE) %>%
  ungroup()

top_cotton_stop_counts %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = "Top words in cotton-related debates after stopword removal (counts)")

```
TF-idf makes stopword lists less necessary because the IDF part reduces the weight of words that appear often across documents. In this case, words that show up a lot in both decades’ cotton debates get a low tf-idf score, and even the highest-scoring stopwords in the stopword check have relatively small tf-idf values. This means it is less likely to influence the results, even without manual removal. Looking at the plot, tf-idf shows us some highly specific terms, especially proper nouns and event labels (such as "miguel," "gosford," and "mulgrave" in the 1830s, and "dockyards," "savoy," and "schleswig" in the 1860s) which likely reflect the broader topics of the debates that happened to mention cotton, not necessarily cotton itself.

On the other hand, using stopword filtering with word counts (as in the table above) answers a different question. It shows which words are most common in each decade’s cotton debates after removing common English words. Tf-idf focuses on what sets the decades apart, highlighting terms that are especially typical of one decade’s cotton debates compared to the other. 

Since these methods answer different questions, they can lead to different interpretations. Using stopword-filtered counts is better for showing the main themes or general content of cotton debates in each decade. Tf-idf, on the other hand, is better for finding what makes each decade’s cotton debates unique. However, neither method is definitively more accurate or superior than the other, because each gives a different view of the historical record. The best approach is to use both as starting points for close reading. If a high tf-idf term shows a real change in how debates are framed or what policies are discussed, you should see it in the debate text. If it is just noise, like names or rare quirks, close reading will reveal that as well.
...
Tokens and bigrams are great for indexing patterns, but they do have real limits for historical meaning, because they rely on a “bag-of-words” approach that treats text as a collection of words without regard to grammar, order, or context, so they can miss how arguments work at the sentence level. A one or two-word approach doesn’t show you the full syntax and context, so it can’t tell you whether a speaker is in favor of the term or against it, quoting an opponent, using a word ironically, using a different meaning of that word (polysemy) or shooting down a claim (ex: if many speakers say “no cotton” you might get a high usage count but it would be misleading). The true context of something is usually revealed from looking at longer phrases or sentences, not in isolated words. Parliamentary speech adds another problem because procedural language and repeated rhetorical scaffolding can dominate word counts even when they aren’t the main point of the argument, and bigrams will still miss longer recurring phrases, idioms, and names that matter historically. 

So, tokens and bigrams are a tried-and-true method, and they are crucial for bringing themes, frames, and speaker differences to the surface. However, they are ultimately most reliable as a first lens of many, and you are best off validating your discoveries by going back to passages, reading them in context, and checking whether the pattern reflects a real historical shift or just a change in style, genre, or dataset structure.
...
**Purpose:** I used AI to troubleshoot R execution errors and RMarkdown knitting failures that happened while running the chapter code. As in previous labs, there was a Corrupt <IDate> memory/vector allocation error that caused left_join to crash. I fixed this by adjusting the speechdate column when loading the hansard 1830 and 1860 datasets and metadata. I also used AI to solve a LaTeX compilation error caused by missing Noto Serif fonts in the YAML header so the document could knit to PDF.

**Example Prompts:** How do I fix these errors? "Error in `vec_proxy()`:! Corrupt <IDate>. Expected integer storage, not double storage. Backtrace..."

"! Package fontspec Error: The font 'Noto Serif' cannot be found..."

**Independent Work:** I structured the R code for the data cleaning, loop setup, and visualizations, intentionally adapting it to align with the textbook's methodology. I also independently wrote all historical claims, the theoretical reflections on TF-IDF versus stopwords, and the close reading analysis of the 1833 Abolition of Slavery debate.

**Explanation of Code**
In Exercise 1, I broke down Chancellor speeches into tokens, calculated tf-idf scores by suggested speaker, and made plots of the top terms for each speaker. For Exercise 2, I created word-frequency lists for each speaker in the 1833 abolition debate, measured pairwise JSD, and selected text samples for close reading. In Exercise 3, I focused on debates mentioning 'cotton,' calculated tf-idf scores by decade, and compared those results to word counts with stopwords removed to highlight the difference between distinctiveness and frequency.
